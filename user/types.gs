 ...
 * @property {number} port The server port. MySQL 3306, PostgreSQL 5432,
 *     and 1433 SQL Server.
 * @property {string} db The default database to connect to.
 * @property {string} prefix The prefix for the connection URL.
 *     (e.g. 'jdbc:mysql://', 'jdbc:postgresql://', 'jdbc:google:mysql://',
 *     or 'jdbc:sqlserver://'.)
 ...
 *     detect the data types of the columns in the table and use them for the
 *     insert operation. If set to `true`, the library will analyze the data
 *     being inserted and determine the appropriate data types for each column.
 *     If detectTypes is set to `false`, all data will be treated as strings,
 *     regardless of the `stringTypes` option. Defaults to `true`.
 * @property {boolean} stringTypes An array of data types that should be treated
 *     as strings (e.g., `integer`, `double`, `object` | `array`, `boolean`)
 *     when the `detectTypes` option is enabled.
 */
 ...
 * @property {string} filter.column The name of the column to filter on.
 * @property {string|number} filter.value The value to match in the specified column
 *     (WHERE '=').
 * @property {string|number} filter.value_from The starting value for
 *     range-based filtering (WHERE '>').
 * @property {string|number} filter.value_to The ending value for range-based
 *     filtering (WHERE '<').
 * @property {boolean} filter.toUnixTime Indicates whether to convert the dates
 *     of the filter values to a timestamp (UNIX format).
 * @property {string} filter.sort The column name to use for sorting.
 * @property {string} filter.direction The sorting direction
 *     ('asc' for ascending, 'desc' for descending).
 * @property {number} filter.limit The maximum number of records to return.
 * @property {number} filter.offset The number of records to skip before returning
 *     results.
 */
